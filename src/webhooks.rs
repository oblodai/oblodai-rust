...
pub fn verify_webhook(
    ...
    if headers.timestamp.is_empty() || headers.signature.is_empty() {
        ...
        log_env(
            LogLevel::Warn,
            "oblodai: webhook verify failed: missing timestamp or signature",
        );
        return Err(Error::Signature(
            "отсутствует timestamp или signature вебхука".into(),
        ));
    }
    ...
    if !constant_time_eq(expected.as_bytes(), headers.signature.as_bytes()) {
        log_env(
            LogLevel::Warn,
            "oblodai: webhook verify failed: signature mismatch",
        );
    ...
    if opts.max_age_seconds > 0 {
        let ts: u64 = match headers.timestamp.parse() {
            ...
            Err(_) => {
                log_env(
                    LogLevel::Warn,
                    "oblodai: webhook verify failed: invalid timestamp",
                );
        ...
        if age > opts.max_age_seconds {
            log_env(
                LogLevel::Warn,
                &format!(
                    "oblodai: webhook verify failed: too old ({age}s > {}s, replay)",
                    opts.max_age_seconds
                ),
            );
            return Err(Error::Signature(
                "вебхук слишком старый (replay-защита)".into(),
            ));
        }
    }
